...
- **NFR-09a Proxy symmetry — a wrapper must answer the SAME question for writes as for reads (v2.04, NFR-09's shape in a new place).** A decorator or proxy whose attribute WRITES forward to the inner object while its READS are intercepted is answering a different question in each direction, and the failure is not subtle-but-harmless: a test spy installed on the wrapper's `submit` was forwarded to the inner broker, so the guard called the spy which called the guard until the stack ran out. **Rule:** names the wrapper DEFINES are written to the WRAPPER; write-through is preserved only for names it does not define (legitimately required — paper cash is reset through the broker reference). Diagnosability note worth keeping: the recursion was findable only because the resolver reported it honestly as `UNKNOWN … positions() failed (RecursionError)` rather than degrading to a default — fail-loud paid for itself.
- **NFR-07a Proof shape follows component type (v2.04).** NFR-07's registry proof is "constructed AND ticked" for loops and monitors; a STRUCTURAL CHOKEPOINT has no task to watch, so its registry entry carries TWO different real proofs instead: that the composition root's broker reference is the guarded one, AND that the canonical close path's own broker reference is the guarded one. The second is the load-bearing proof — it demonstrates the close actually routes through the guard rather than holding a raw reference captured earlier in construction.
- **NFR-12 An ABSENCE is pinned at SOURCE, because it cannot be observed at runtime (v2.06, live-observed).** Removing exit evaluation from the health tick **broke no test** — the wiring was never pinned, which is exactly how the code drifted from what TPF-03 already required ("every valid quote evaluation") with nothing checking. The reason a runtime test cannot cover it is decisive: **"this loop no longer evaluates exits" is indistinguishable from "it evaluated and nothing breached."** Therefore where a rule constrains an OUTCOME (TPF-03a explicitly permits a loop OR a subscriber), tests pin the outcome; but the required ABSENCE is pinned AT THE SOURCE. Corollary — **silence is indistinguishable from safety**: an exit evaluator that stops evaluating looks exactly like an entry that never breached, which is why TPF-03a's loop carries a done-callback making its death a CRITICAL alert rather than a quiet stop.
- **NFR-11 Wiring is CAPTURE-TIME, not attribute-time (v2.05, live-observed).** Assigning to a composition attribute after construction (`comp.x = real_x`) rebinds the CONTAINER's attribute and does NOT reach any reference a component already captured. Two live instances of this single root cause: (i) the ORD-12 guard, which had to be installed innermost BEFORE any service captured the broker reference; (ii) the alert sink, installed late by a plain `comp.alerts = alerts`, which left **ProtectPosition's STP-04 critical, ExecuteEntryAttempt's lost-submit critical and CloseEntry's CLS-06 partial-close critical DEAD in live AND paper** — two of the three were constructed with no `alerts=` kwarg at all, so nothing could surface it. **Rule:** a collaborator whose identity matters is supplied AT CONSTRUCTION, or it is a stable relay object that RETARGETS IN PLACE and is never replaced. The `AlertRelay` takes the second form deliberately: handed out at construction, retargeted in place, and it RECORDS AND REPLAYS anything raised before a target exists so construction-time alerts are not lost. That retarget-in-place read/write asymmetry is the ratified contract, NOT an instance of NFR-09a — there the asymmetry was accidental and recursed; here stable identity IS the mechanism, it is pinned by test, and it must not be "simplified away".
- **NFR-11a A gate that cannot prove a thing must say WHY (v2.05).** The NFR-07 registry's `_isinstance_check` performs a FLAT `getattr`, so a dotted proof path silently proves nothing and reports "unconstructed" forever without ever stating that it could not resolve the path. Failing closed is correct; failing SILENTLY is not. A proof that cannot be evaluated is reported as UNEVALUABLE with its reason, never as a plain negative — same principle as TPF-03d surfacing unevaluable exits and NFR-09's fail-loud. Until the checker resolves dotted paths, proofs are written explicitly rather than dotted.
- **NFR-10 Git operations on a real-money tree are single-step and verified (v2.03, from a self-reported near-miss).** Multi-step git state operations MUST NOT be chained in one compound command on the working tree of a live trading system. Recorded incident (2026-07-26, no loss): a `git stash push` failed on an untracked observation file and the `git stash pop` chained after it in the same command popped the PRE-EXISTING stash instead, spilling a rejected build into the tree; nothing was lost only because the failure happened to be the safe one, and recovery was clean because the operator moved the leaked files aside rather than deleting them. Rule: each state-changing git step is issued separately and its result verified before the next; recovery prefers MOVING files aside over deleting them; and a compound command whose second step assumes the first succeeded is forbidden. Same principle as the ratified preference for a clean rebuild over git surgery (v1.92) — on a real-money tree, cleverness is the risk.
- **NFR-09 Liveness predicates use DENY-LISTS, never allow-lists (v1.92, generalised from the STP-04a investigation).** Any predicate answering *"is this thing still live?"* — a working order, a position, a session, a subscription — is built as a **deny-list of known-terminal states plus an explicit UNKNOWN branch**, never an allow-list of known-live states. An allow-list fails toward **absent**, and absent is what triggers destructive action: the live `working_orders()` allow-list silently dropped a real resting stop at status `Routed` (plus four other fillable statuses), so a PROTECTED position read as unprotected and was AUTO-FLATTENED. An unrecognised state resolves UNKNOWN and is logged loudly — never treated as gone. **The method, sharpened again (v2.01 — PRODUCE THE EVIDENCE, don't reason from shape):** naming the question a predicate answers is necessary but not sufficient. The rule is to **produce the artifact that answers it** — the enum dump, the call-site census, the recorded payload. Recorded record: every time that was done the story changed (`.endswith("filled")` LOOKED like it caught partials, the enum says it cannot; `fills_since(cursor)` LOOKED like a dropped parameter, all thirteen callers say it is dead), and every time a defect was described by reasoning from shape, the description was wrong — twice, in the same direction. An unobserved assumption is not a smaller version of an observed fact; it is the thing that has been wrong every time so far.

**The diagnostic (v1.93 — audit for the SHAPE, not the symptom):** every instance so far is *a predicate that answers a NARROWER question than its name implies, and whose failure mode is silence.* `working_orders` answered "is it in my known-live list" instead of "is it still live"; `fill_legs` answered "what legs are on this order" instead of "what filled". Both fail toward absent; absent triggers destruction. **`positions()`, `fills_since()`, and `find_matching_order()` MUST be audited against that framing specifically** — name the question each predicate actually answers, compare it to the question its callers believe they are asking, and make the gap explicit or close it: a question about existence must fail toward *present*, because the safe error is hesitating over something real, never acting on something imagined. Ratified classification for orders: WORKING = received, live, routed, contingent, in flight, cancel requested, replace requested, partially removed, **plus anything unrecognised**; DEAD = cancelled, rejected, expired, removed, filled.
- **NFR-08a Exit-evaluation failures ALERT, never log-only (v1.94).** A raised exception from the exit evaluation pass MUST produce a CRITICAL alert, rate-limited to at most one per `config.exit_unevaluable_alert_s` per distinct error, and MUST NOT be reported by log line alone. The evaluation loop survives the exception. Rationale: the call site caught everything and emitted a warning only, so a throwing evaluator left every exit dead for a session with no operator-visible signal — the exact incident behind NFR-08 (an exit evaluator threw on 15 consecutive health ticks while all exits were dead and the session continued). At a 250 ms cadence a silent throw is 240× more frequent and no more visible.
- **NFR-08 Alert sinks must be WIRED, and a thrown evaluation must be heard (v1.90).** Every component that can raise an alert is constructed WITH its alert sink; a `None` sink in any live/paper composition is a CI failure under the NFR-07 registry (verified instance: `ExecuteEntryAttempt` was constructed without `alerts=` in both compositions, so the lost-submit, stale-fill, and unconfirmable-terminal alerts were ALL DEAD in production — safety machinery believed to be watching was not). Additionally: an exception thrown inside a monitor's evaluation MUST surface as an alert, never a silent no-op — corroborating evidence (external operator, 2026-07-20): an exit evaluator threw on FIFTEEN consecutive health ticks while TPF/TPT/close were entirely non-functional, and the session simply continued.
