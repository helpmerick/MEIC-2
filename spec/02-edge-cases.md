...
- **EC-LEX-08 No bid at all (v1.63, operator-ratified).** If at the end of `lex_quote_wait_seconds` the side's long has NO bid: (a) with a DAT-02-fresh underlying mark, place a resting limit sell at max(one minimum tick, intrinsic floored to tick) — the LEX-04 floor evaluated with bid = 0, generalizing EC-LEX-04 — resting until fill, quote resumption, or expiry (EOD-04); **placing this floor order raises the one-time critical alert** (operator addition: a degraded-liquidity recovery is announced, not discovered); (b) a usable quote arriving later supersedes the floor order via the raced-fill-guarded cancel/replace (LEX-08), resuming LEX-03 pricing; (c) with NO underlying mark either (no intrinsic computable), defer with the one-time critical alert — with no bid and no spot, any price is invented. Exposure during deferral is long-only, defined-risk (LEX-06 classification). **(d) Restart survival (v1.64, from final review):** on boot, any resting intrinsic-floor sell is RE-ADOPTED via its `lex-floor:` idempotency key (REC-05 pattern) — registry loss on restart must never orphan a floor order: an unrediscovered working order breaks supersession (long sells at intrinsic when it could ladder a returned bid), misclassifies its fill as OWN-standdown, and corrupts the EOD-03 audit. REC-03's "resume" applies to floor orders exactly as to ladders. **(e) Fill-while-down recognition (v1.65, from the v1.64 review escalation — the EC-STP-06 principle applied to floor orders):** on boot, a journaled `kind="floor"` order ABSENT from working orders whose broker fill record shows FILLED has its `LongSold` synthesized at the broker-actual fill price (ORD-09), closing the side terminally — never perpetual-pending, never OWN-standdown misclassification, never an EOD-03 audit short one event. Only a floor order genuinely gone WITHOUT a fill (externally cancelled) takes the OWN paths. This makes STP-08a's "never waits indefinitely" true whenever a floor is computable, and honestly bounded when not.
