...
- **RPT-17 Day-trades table & the Unmanaged counterfactual (v1.82, operator-commissioned — pulled forward through the freeze by explicit operator fiat, logged as such).** (1) **Day-trades table (UI-33):** the TRADING tab carries, at the bottom, a table of TODAY'S entries — live and closed — one row per entry: entry time (ET), source (bot/manual initiator), target premium, actual net credit, wing width, per-side short/long strikes with the day's SPX reference (close once settled, latest recorded spot before), per-side status badges (open/protected/stopped/closed/expired/decay), stop-fill count, and realized P&L net of fees (PNL-01, ORD-09a broker-actual; open entries show live P&L per UI-13, badged unrealized). Day total row. All numbers from the ONE aggregation path (RPT-09a) — this view recomputes nothing. Settlement-pending days render the EOD-01 PROVISIONAL label, never fake finality. SPX H/L, when shown, derive from RECORDED spot samples only, labeled as such — never fetched retrospectively. (2) **Timing & Unmanaged report:** per entry — opened/closed ET times (ORD-11), duration, premium, realized P&L, and **Unmanaged P&L** = premium received minus the entry's spread value at 16:00 ET computed FROM RECORDED SAMPLES (see 3), i.e. what doing nothing would have made. It is DISPLAY/ANALYTICS ONLY: never a trading input, never a correction source, and rendered with the honest caption stating its definition. Missing samples ⇒ "no data (not sampled)", never interpolated (D10). (3) **Sampler extension (D8b):** the EntryMarkSample recorder continues sampling ALL of today's entries' legs until the 16:00 close EVEN AFTER an entry closes (bounded: same 1-minute cadence, day-scoped, replay-safe) — the counterfactual is computed from recorded observations, not from a new historical-quote dependency.
