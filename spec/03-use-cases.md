...
- **UI-18a Markup per-row (v1.61).** The `stop_rebate_markup` dial is exposed PER-ROW in the schedule editor (doc 06 §37 override), validated reject-never-clamp, and carries BOTH disclosures whenever markup > 0: the UI-18 shortfall sentence AND the dollar worst-case increase = markup × 100 × contracts × 2 (both stops carry the markup).
...
- **UI-23a Local label (v1.61).** The UI-23 echo's label names the browser's configured timezone (resolved city/zone, e.g. "≈ 16:53 London") — it is the browser's Intl setting, NOT geolocation: nothing is looked up, requested, or tracked.
