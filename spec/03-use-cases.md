...
- **UI-34 Event proximity warning banner (v1.84).** Dismissable warning line(s) at the top of the Trading tab per CAL-11: names the event and its proximity (day-of, or T-1/2/3 trading days), stacks nearest-first, tier-2 events labeled best-effort. Display-only — never gates or blocks anything, visually distinct from the STP-04 UNPROTECTED / critical banners (an event warning is amber-informational, never red-critical). Dismiss is per-event-per-tier, persisted (REC-07); the nearest-proximity warning is never pre-dismissed by an earlier-tier dismissal.
- **UI-35 Terminal-state wording (v1.89).** A never-filled entry NEVER renders as "Closed" — "Closed" asserts a position existed and was closed. Cards and tables show the entry's actual terminal state, matching its drilldown exactly: **Unfilled** (priced out at the floor), **Cancelled** (operator cancel), **Insufficient credit**, etc. `already_terminal` is ratified vocabulary for a re-issued command against an entry that has already reached a terminal state.
- **UI-33 Day-trades table on the Trading tab (v1.82, operator-commissioned).** Bottom of the Trading tab: today's entries, live and closed, per RPT-17(1) — columns per that rule, live rows updating in place, closed rows joined by the Timing & Unmanaged report (RPT-17(2)). Colorblind-safe per UI-26 (status badges shape+color); every money figure hover-exact (Decimal); tooltips per the UI-31 standard (jargon defined). Read-only — no trading controls inside the table beyond each row linking to its entry card.
...
- **UI-32 Getting-started tab (v1.75).** DOC-06's setup guide is the FIFTH TAB. Nav becomes Trading | Results | Calendar | How it works | Getting started. Read-only, no trading controls, never renders env values or secrets — annotated template and instructions only; version-stamped per DOC-05.
- **UI-30 Calendar tab (v1.71).** The trading calendar (doc 11, CAL-08) is the THIRD TAB in the SPA's top-level navigation — Trading | Results | Calendar | How it works | Getting started (operator-specified five-tab layout, v1.75). Year view, tagging, and category rules per CAL-08; the trading tab surfaces the active day's tag and skip reasons render tag labels.
