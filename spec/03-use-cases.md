...
- **UI-24 Next-entry countdown (v1.53).** While ARMED with entries remaining, the panel shows the next scheduled entry (ET time + UI-23 local echo) and a live countdown. DISARMED ⇒ "schedule idle — arm to run"; ARMED, none left ⇒ "no more entries today". DISPLAY-ONLY (UI-03): the authoritative next-entry time and remaining seconds come from the backend (`/day/status`, DAY-03-verified clock); the client only animates between polls — never the browser clock.
- **UI-23 Local-time echo (v1.53).** Beneath each ET entry time the schedule panel displays the operator's local-timezone equivalent, resolved live from the browser (`Intl`) so DST is automatic. ET remains the system of record (DAY-03); the echo is display-only — never persisted, never a trading input. An invalid time shows the precise reason instead (not_24h_military / outside_market_hours).
...
**Flow:** With an open condor, operator presses "Simulate bot outage" → backend disconnects its own broker/data sessions for `config.drill_outage_seconds` (default 60) → UI (on reconnect) shows evidence that the stop orders remained working at the broker throughout (order timestamps unbroken), plus a clean reconcile (no duplicates per REC-05, no re-placed stops). **Live mode (v1.56, operator-ratified):** available with the operator present, behind a typed DRILL confirmation; the dialog SHOULD warn if short marks sit within 50% of trigger distance or an entry fires within 10 minutes (guidance, not a hard block — the operator is supervising). Paper-mode drills remain available but prove less (SIM-03 honesty note). The drill NEVER touches orders — it only severs the bot's own sessions.
