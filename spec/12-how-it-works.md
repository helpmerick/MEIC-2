...
**v2.03 (operator-commissioned 2026-07-15; current through spec v2.03).** A complete, non-technical
...
- **DOC-05 Rendering (UI-29).** A separate client-side route in the existing
  ...
  the page banners the mismatch instead of pretending currency. **Readable diagrams (v1.77, operator-ruled — "the text is so small I can't see"):** the master flowchart and every diagram in the guide render CLICKABLE and ZOOMABLE — click expands to a full-screen view with pan and scroll/pinch zoom, plus explicit zoom controls; default rendering must meet UI-26 contrast and never rely on squinting; the full-screen view is keyboard-dismissable.

---

# THE GUIDE (ratified content, v2.03 — describes spec v2.03; DOC-05 stamp)
...
# GETTING STARTED (ratified content, v2.03 — describes spec v2.03 and the build's true run procedure; DOC-05 stamp)
