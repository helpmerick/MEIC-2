...
# dxfeed Profile.trading_status's tradeable value (the other possibilities
# are "HALTED" and "UNDEFINED" — anything not exactly this is treated as
# halted, per DAT-04a clause 2: "status != active ... => halted").
...
class TradingStatusStore:
    ...
    def record(self, status: str, at: datetime) -> None:
        ...
        if self._reading is not None and at < self._reading.at:
            return
        self._reading = TradingStatusReading(status=str(status).strip().upper(), at=at)
    ...
    def halted(self, now: datetime) -> bool:
        """DAT-04a: no reading at all, a non-ACTIVE status, or a reading
        stale beyond `HALT_READING_STALE_AFTER_SECONDS` all mean halted —
        unmeasured = unverified = blocked (RSK-07), uniform with the other
        three ENT-03 gate inputs `LiveMarketGates` sources."""
        reading = self._reading
        if reading is None:
            return True
        if reading.status != ACTIVE_STATUS:
            return True
        age_seconds = (now - reading.at).total_seconds()
        return age_seconds > HALT_READING_STALE_AFTER_SECONDS
