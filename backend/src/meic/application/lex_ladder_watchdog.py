...
def _pending_ladder_starts(events: list[Event]) -> set[tuple[str, str]]:
    ...
    closed at the entry level by any other GENUINE (non-stale-able) path
    (`EntryClosed` whose `initiator` is not a member of the shared
    `STALE_TERMINAL_INITIATORS` set, `meic.domain.projection`).

    `close_entry.py` used to be the ONLY writer of `EntryClosed` -- that
    stopped being true at spec v1.87/v1.88: `ManualClose.cancel_working` also
    writes `EntryClosed(initiator="cancelled")` for a CLS-03(a) pre-fill
    cancel, and `execute_entry.py`'s `_skip` writes
    `EntryClosed(initiator="unfilled")` / `EntryClosed(initiator=
    "cancelled_by_operator")` for the v1.88 ladder terminals -- all three
    dispose of NOTHING (a genuinely-nothing-filled entry has no legs, so it
    can never appear in `stop_initiator` below anyway -- this is a no-op in
    the normal case). But a fill can surface AFTER any of these terminals was
    journaled (the terminal's own fills-feed check missed it before it
    propagated) -- the LATE-FILL ANOMALY: `projection.apply` clears the
    now-stale `close_initiator` and a `ReconciliationMismatch` + critical
    alert fire (REC-01/CLS-03(a)/(a2) v1.87/v1.88), but the entry genuinely
    holds a position and can genuinely stop out. Honouring a stale terminal
    here -- treating it the same as a real CLS close -- would BLIND this
    backstop against exactly the orphaned-long class it exists to catch (the
    2026-07-10 incident this whole module was written for, and the exact
    LEX-07 critical alert this module fires). A member of
    `STALE_TERMINAL_INITIATORS` is therefore never counted as "this entry was
    disposed of" by this watchdog; only a genuine, non-stale-able close is
    (`getattr` for replay-safety against any legacy event lacking the field).
    This is deliberately the SAME shared set `execute_entry.py`'s own
    late-fill staleness check uses -- a THIRD consumer hardcoding its own
    literal here (as this module used to, checking only `!= "cancelled"`)
    is exactly the drift `STALE_TERMINAL_INITIATORS` exists to prevent; see
    `tests/application/test_tc_cls_07.py`'s drift-guard tests.
    ...
    for e in events:
        if isinstance(e, ShortStopped):
            ...
            terminal.add((e.entry_id, e.side))
              ...
              and getattr(e, "initiator", None) not in STALE_TERMINAL_INITIATORS):
