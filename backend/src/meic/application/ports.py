...
class BrokerGateway(Protocol):  # implemented by TastytradeAdapter, FakeBroker
    ...
    # ENT-11(9) v2.00: the `cursor` parameter is REMOVED, not implemented --
    # every caller passed None, so the signature promised incremental delivery
    # no implementation ever gave. The name is kept for its 13 call sites; the
    # honest contract is "the broker's current filled-orders view".
    async def fills_since(self) -> list[Fill]: ...
